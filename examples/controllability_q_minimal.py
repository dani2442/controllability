...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--T", type=float, default=120.0, help="Final horizon")
    parser.add_argument("--dt", type=float, default=0.1, help="Time step")
    parser.add_argument("--seed", type=int, default=42)
    parser.add_argument("--beta-scale", type=float, default=0.0, help="Process noise scale")
    parser.add_argument("--delta-scale", type=float, default=0.0, help="Measurement noise scale")
...
def main() -> None:
    ...
    axes[0].plot(idx, np.array(pi_min_eigs), marker="o")
    axes[0].set_title(r"$\lambda_{\min}(\widehat{\Pi}_{L,K}(\lambda))$")
    ...
    axes[1].semilogy(
        idx,
        np.maximum(np.array(pi_rel_errors), 1e-16),
        marker="o",
        color="tab:red",
    )
    axes[1].set_title(r"$\|\widehat{\Pi}_{L,K}(\lambda)-\Pi^{\mathrm{mod}}_{L,K}(\lambda)\|_2 / \|\Pi^{\mathrm{mod}}_{L,K}(\lambda)\|_2$")
    axes[1].set_xlabel("Candidate index")
    axes[1].set_ylabel("Relative model mismatch")
