"""Convergence of basis-free Pi-errors under stochastic noise.

This example estimates the rate

    ||Pi_hat_{L,K}(lambda; T) - Pi_mod_{L,K}(lambda)||_2 = O_P(T^{-1/2})
...
For each horizon T_i and noisy run j, define
    ...
    e_j(T_i) = max_{lambda in Lambda_cand}
               ||Pi_hat^{(j)}_{L,K}(lambda; T_i) - Pi_mod_{L,K}(lambda)||_2.
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Convergence of Pi_hat-to-Pi_mod errors under stochastic noise."
    )
    parser.add_argument("--system", type=str, default="random", choices=list(SYSTEMS.keys()))
    parser.add_argument("--T", type=float, default=1000.0, help="Final horizon")
    parser.add_argument("--dt", type=float, default=0.1, help="Time step")
    parser.add_argument("--min-T", type=float, default=20.0, help="Minimum horizon")
    parser.add_argument("--num-horizons", type=int, default=8)
    parser.add_argument("--mc-runs", type=int, default=20, help="Monte Carlo noisy realizations")
    ...
    parser.add_argument("--smooth-y", action=argparse.BooleanOptionalAction, default=True)
...
def main() -> None:
    ...
    print("\nEvaluating worst-case Pi-errors per horizon...")
    for i, Ti in enumerate(horizons):
        ...
        clean_errors[i] = worst_case_pi_error(
    ...
    print("\nSummary table (worst-case over fixed candidate lambdas):")
    ...
    ax1.set_ylabel(r"$\max_{\lambda}\|\widehat{\Pi}(\lambda;T)-\Pi^{\mathrm{mod}}(\lambda)\|_2$")
    ax1.set_title("Pi-error decay")
