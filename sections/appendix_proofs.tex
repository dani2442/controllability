...
\begin{proof}
  ...
  Using \eqref{eq:hautus-continuous-factorization}, this gives $v^*\mat{H}_\lambda(u)=v^*\mat{P}_\lambda\mat{S}_Z(u)=0$, hence $\operatorname{rank}(\mat{H}_\lambda(u))<n$.
  This proves the contrapositive of the first implication in Theorem~\ref{thm:hautus-margin-necessary}.

  Conversely, assume $(\mat{A},\mat{B})$ is controllable and $\mat{S}_Z(u)$ is invertible.
  Then $\mat{P}_\lambda$ has full row rank for every $\lambda\in\CC$ and \eqref{eq:hautus-continuous-factorization} implies
  \[
    \operatorname{rank}(\mat{H}_\lambda(u))
    =\operatorname{rank}(\mat{P}_\lambda\mat{S}_Z(u))
...
\subsection{Proof of Corollary~\ref{cor:hautus-finite-checking-H}}\label{app:proof:hautus-finite-checking-H}
\begin{proof}
  Suppose there existed $\lambda_0\in\CC$ with $\operatorname{rank}\big(\mat{H}_{\lambda_0}(T)\big)<n$.
  Since $\mat{S}_Z(u)\succ 0$, the matrix
  \[
    \mat{G}_{\lambda_0}(u)=\mat{H}_{\lambda_0}(T)\mat{S}_Z(u)^{-1}\mat{H}_{\lambda_0}(T)^*
  \]
  is singular, hence \eqref{eq:hautus-operator-full-rank} implies $\operatorname{rank}(\mathcal{P}(\lambda_0))<n$.
  Theorem~\ref{thm:hautus-operator-finite-lambda} then yields $\lambda_0\in\sigma(\mat{K})=\Lambda$, contradicting the hypothesis.
\end{proof}
...
\begin{proof}
  ...
  \]
  Since $\mat{S}_Z(u)\succ 0$, \eqref{eq:hautus-cross-moment-G-from-H} gives
  \[
  ...
  \[
    ...
    =w^*\mat{H}_\lambda(T)\mat{S}_Z(u)^{-1}\mat{H}_\lambda(T)^*w
    \le \frac{1}{\sigma_{\min}(\mat{S}_Z(u))}\,\|\mat{H}_\lambda(T)^*w\|_2^2,
  \]
  i.e.,
  \[
    \|\mat{H}_\lambda(T)^*w\|_2^2 \ge \sigma_{\min}(\mat{S}_Z(u))\,w^*\mat{G}_\lambda(u)w.
  \]
