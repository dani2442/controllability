...
\begin{proof}
  ...
  \[
    \E\big[\mat{M}(T)_{i,:}^\top \mat{M}(T)_{i,:}\mid z\big] = \int_0^T z(t)z(t)^\top\,\d t = \widehat{\mat{S}}_Z(T),\qquad i=1,\dots,q.
  \]
  Therefore, conditional on $z$, we have the distributional identity $\mat{M}(T)\stackrel{d}{=}\mat{G}\widehat{\mat{S}}_Z(T)^{1/2}$ where $\mat{G}\in\R^{q\times (n+m)}$ has i.i.d.\ $\mathcal{N}(0,1)$ entries.
  Using \eqref{eq:hautus-cross-moment-error},
  \[
    \widehat{\mat{P}}_\lambda(T)-\mat{P}_\lambda
    \stackrel{d}{=}
    \beta\,\mat{G}\widehat{\mat{S}}_Z(T)^{-1/2},
  \]
  conditional on $z$, hence
  \[
    \|\widehat{\mat{P}}_\lambda(T)-\mat{P}_\lambda\|_2
    \le
    \frac{\|\beta\|_2}{\sqrt{\sigma_{\min}(\widehat{\mat{S}}_Z(T))}}\;\|\mat{G}\|_2.
  \]
  The standard Gaussian matrix bound $\mathbb{P}\big(\|\mat{G}\|_2\ge \sqrt{q}+\sqrt{n+m}+t\big)\le e^{-t^2/2}$ (valid for all $t\ge 0$; see, e.g., \cite{vershyninIntroductionApplicationsData}) yields \eqref{eq:hautus-cross-moment-high-prob} by choosing $t=\sqrt{2\log(1/\delta)}$ and using $\sigma_{\min}(\widehat{\mat{S}}_Z(T))=T\sigma_{\min}(\bar{\mat{S}}_Z(T))$.
\end{proof}
