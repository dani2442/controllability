...
\[
  \d x(t) = (\mat{A}x(t) + \mat{B}u(t))\,\d t + \boldsymbol{\beta}\,\d W(t),
\]
where $\mat{A}\in\R^{n\times n}$ is the drift matrix, $\mat{B}\in\R^{n\times m}$ the input matrix, $\boldsymbol{\beta}\in\R^{n\times q}$ the noise intensity, and $W$ a $q$-dimensional standard Brownian motion.
...
Our first experiment validates the data-driven Hautus test by comparing the estimated matrix $\widetilde{\mat{P}}_\lambda$ with the true Hautus matrix $\mat{P}_\lambda = [\mat{A}-\lambda\mat{I}, \, \mat{B}]$.
...
Both methods yield accurate estimates, with estimation errors $\|\widetilde{\mat{P}}_\lambda - \mat{P}_\lambda\|_2$ on the order of $10^{-2}$.
...
Figure~\ref{fig:hautus-error-summary} (left) summarizes the signed error $\sigma_{\min}(\mat{P}_\lambda) - \sigma_{\min}(\widetilde{\mat{P}}_\lambda)$, showing near-zero bias and comparable variance for time-domain and FFT estimators.

\begin{figure}[ht]
  \centering
  \includegraphics[width=0.95\textwidth]{images/hautus_matrix_comparison.pdf}
  \caption{Element-wise comparison of the true Hautus matrix $\mat{P}_\lambda$ (left) and its time-domain estimate $\widetilde{\mat{P}}_\lambda$ (center), with absolute difference (right). The small difference magnitudes confirm the accuracy of the cross-moment estimator.}
...
Proposition~\ref{prop:hautus-cross-moment-high-prob} predicts that the estimation error $\|\widetilde{\mat{P}}_\lambda - \mat{P}_\lambda\|_2$ decays at the rate $\mathcal{O}(T^{-1/2})$ as the observation horizon~$T$ increases.
...
\]
In the noise-free case $\d x(t)=\dot x(t)\,\d t$, so $\mat{M}_X$ reduces to $\int_0^T x(t)\dot x(t)^\top\,\d t$; under It\^o noise, $\mat{M}_X$ is estimated directly from state increments and includes an additional martingale term.
...
For each candidate $\lambda_i$, we compute the estimated minimum singular value $\sigma_{\min}(\widetilde{\mat{P}}_{\lambda_i})$ and compare with the true value $\sigma_{\min}(\mat{P}_{\lambda_i})$.
...
Since $\sigma_{\min}(\widetilde{\mat{P}}_\lambda) > 0$ for all candidates, the system is correctly identified as controllable.

\subsection{Comparison of Time-Domain and FFT Methods}
\label{subsec:exp-method-comparison}

We compare two computational approaches for evaluating the cross-moments: (i)~the \emph{time-domain method}, which computes $\widetilde{\mat{H}}_\lambda(T)$ via direct time stepping, and (ii)~the \emph{FFT method}, which leverages frequency-domain representations (via \eqref{eq:hautus-fourier-y} and Parseval-type identities).
...
\begin{figure}[t]
  ...
  \begin{subfigure}[b]{0.48\textwidth}
    ...
    \begin{tabular}{@{}ccc@{}}
      \toprule
      $\omega_{\max}$ & $\sigma_{\min}(\widetilde{\mat{P}}_\lambda)$ & $\|\widetilde{\mat{P}}_\lambda - \mat{P}_\lambda\|_2$ \\
      \midrule
      10 & 0.412 & 0.089 \\
      20 & 0.428 & 0.051 \\
      50 & 0.435 & 0.027 \\
...
In terms of computational efficiency, the FFT method can be advantageous when evaluating many $\lambda$ values, since $\widehat{x}(\ii\omega)$ and $\widehat{z}(\ii\omega)$ can be computed once and reused across all $\lambda$.
...
This persistent excitation ensures that the stacked Gramian $\widetilde{\mat{S}}_Z(T)$ is well-conditioned, which is crucial for accurate estimation of the Hautus matrix and reliable controllability certification.
