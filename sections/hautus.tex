...
In Section~\ref{subsec:hautus-time-domain}, we introduce a residual Gramian $\mat{G}_\lambda(u)$ and show that its invertibility for all $\lambda\in\CC$ is equivalent to controllability under a mild data-richness condition.
Section~\ref{subsec:hautus-ito} develops a derivative-free cross-moment formulation---applicable in both deterministic and It\^o settings---that enables estimating $\mat{G}_\lambda(u)$ and a corresponding Hautus margin directly from measured data.
...
Finally, Sections~\ref{subsec:hautus-best-conditioning-l2} and~\ref{subsec:hautus-best-conditioning-h1} discuss optimal input design principles for conditioning the input Gramian under $L^2$ and $H^1$ budgets, respectively.
...
If moreover $\mat{S}_Z(u)$ is invertible, then the continuous-time Gramian admits the representation
...
%   Assume $x$ is absolutely continuous, $\beta=0$, and $\mat{S}_Z(u)$ is invertible. Then,
%   \[
...
% \end{cor}
% \begin{proof}
% By \eqref{eq:hautus-cross-moment-factorization} and $\beta=0$, 
...
\end{cor}
\begin{proof}
  See Appendix~\ref{app:proof:hautus-operator-finite-checking}.
\end{proof}
...
Consequently, for rank certification it is enough to check the finitely many candidates $\sigma(\mat{K})$.
  \begin{rmk}[Frequency-domain calculation]\label{rmk:frequency-domain-candidate-lambda}
	  The matrix $\mat{K}$ in Theorem~\ref{thm:hautus-operator-finite-lambda} can be computed from the Fourier transforms of $x$ as
	  \[
	    \mat{K}
...
\label{subsec:hautus-best-conditioning-l2}

The data-driven Hautus margins established in Corollary~\ref{cor:hautus-cross-moment-test} are controlled---up to the model-dependent factor $\sigma_{\min}(\mat{P}_\lambda)$---by the smallest singular value of the stacked Gramian $\mat{S}_Z(u)$.
...
\begin{prop}[Best conditioning of the input Gramian under $\|u\|_{L^2}\leq 1$]\label{prop:hautus-isotropic-input}
...
\begin{prop}[$H^1$-budget isotropic input]\label{prop:hautus-isotropic-input-h1}
