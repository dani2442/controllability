...
For $\lambda\in\CC$ define the residual signal
\[
  m_\lambda(t):=\dot x(t)-\lambda x(t)\in\CC^n,
\]
and its cross-moment
\[
...
% \end{equation}
% This yields a direct frequency-domain analogue of the time-domain margin $\sigma_{\min}(\mat{H}_\lambda(T))$.
% In practice, one evaluates $\sigma_{\min}(\mat{H}_\lambda(T))$ on a finite set of $\lambda$ (e.g., a grid or the candidate values discussed below).
