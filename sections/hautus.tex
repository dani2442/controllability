\section{Deterministic Framework for Controllability Certification}\label{sec:problem}
...
\begin{prop}[Best conditioning under $\|u\|_{L^2}\leq 1$]\label{prop:hautus-isotropic-input} Let us define the following optimization problem:
  \[
  \max_{u\in L^2(0,T; \mathbb{R}^m)} \lambda_{\min}\big(\mat{S_U(u)}\big)\qquad \text{s.t.}\quad \|u\|_{L^2(0,T;\mathbb{R}^m)}^2\leq 1,
  \]
  then
  \[
...
\begin{prop}[Best conditioning under $\|u\|_{H^1}\leq 1$]\label{prop:hautus-isotropic-input-h1}
  ...
  \[
  \max_{u\in H^1(0,T; \mathbb{R}^m)} \lambda_{\min}\big(\mat{S_U(u)}\big)\qquad \text{s.t.}\quad \|u\|_{H^1(0,T;\mathbb{R}^m)}^2\leq 1,
  \]
