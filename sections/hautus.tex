...
In Section~\ref{subsec:hautus-time-domain}, we introduce a residual $\mat{H}_\lambda(u)$ and show that its invertibility for all $\lambda\in\CC$ is equivalent to controllability under a mild data-richness condition.
...
\subsection{Noise-free formulation}
...
\begin{equation}\label{eq:hautus-cross-moment-margin}
  \sigma_{\min}\big(\mat{H}_\lambda(u)\big) = \sigma_{\min}(\mat{P}_\lambda\mat{S}_Z(u))
  \ge
  \sigma_{\min}(\mat{P}_\lambda)\,\sigma_{\min}\big(\mat{S}_Z(u)\big), \qquad \forall \lambda\in\CC.
\end{equation}
