...
\begin{itemize}
  ...
  \[
    \d x(t) = \big(\mat{A}x(t) + \mat{B}u(t)\big)\,\d t + \beta\,\d W(t),
  \]
  where $W$ is a standard Brownian motion. In Proposition~\ref{prop:hautus-cross-moment-high-prob}, we prove that
  \[
    |\sigma_{\min}(\widehat{\mat{P}}_\lambda(T)) - \sigma_{\min}(\mat{P}_\lambda)| = \mathcal{O}\left(\tfrac{1}{\sqrt{T}}\right),
  \]
  where $\mat{P}_\lambda:=[\mat{A}-\lambda \mat{I}, \, \mat{B}]$, $\widehat{\mat{P}}_\lambda(T)$ is the data-driven estimated using the noisy model. In other words, the estimation error of the Hautus test decays at the rate $\mathcal{O}(T^{-1/2})$ as the observation horizon $T$ increases.
\end{itemize}
