...
\begin{itemize}
  \item \textbf{Structured persistent excitation for rank recovery.}
  We analyze three families of inputs---polynomial, exponential, and sinusoidal---and prove that they are persistently exciting in the sense that they generically reveal the rank $r$ of the controllability matrix from finitely many snapshots.
  We show that a \emph{single} trajectory generated by a suitable linear combination within these families suffices to recover the full controllable subspace \emph{almost everywhere} in the input parameters, thereby reducing the need for multiple independent experiments.
  ...
  We discuss simple $L^2$-budget design principles for conditioning the Hautus margin $\sigma_{\min}(\mat{G}_\lambda(u))$, based on the input Gramian $\mat{S}_U(u)=\int_0^T u(t)u(t)^*\,\d t$. We show that orthogonal function families (polynomials, exponentials, sinusoids) yield well-conditioning and large Hautus margin uniformly over all $\lambda\in\CC$.
\end{itemize}
%
