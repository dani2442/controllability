...
\begin{itemize}
  ...
  Moreover, it suffices to check a finite set $\lambda \in \sigma(\mat{K})$, where
  \[
  ...
  (2) Under an $H^1$ smoothness budget $\|u\|_{H^1}\le 1$, an optimal design is given by
  \[
    u(t) = \sqrt{\alpha}\,Q\begin{bmatrix}\psi_0(t)\\ \vdots\\ \psi_{m-1}(t)\end{bmatrix},
    \quad\text{where}\quad
    \psi_0(t):=\tfrac{1}{\sqrt{T}},\qquad
  ...
  % \]
  % where $\mat{P}_\lambda:=[\mat{A}-\lambda \mat{I}, \, \mat{B}]$, $\widehat{\mat{P}}_\lambda(T)$ is the data-driven estimate and $\bar{\mat{S}}_Z(T):=\frac{1}{T}\int_0^T z(t)z(t)^\top\,\d t$ is its normalized Gramian.

    \item \textbf{Concentration bounds under It\^o noise.} Consider the noisy LTI model given by
  \[
    \d x(t) = \big(\mat{A}x(t) + \mat{B}u(t)\big)\,\d t + \beta\,\d W(t),
  \]
  where $W$ is a $q$-dimensional standard Brownian motion. In Proposition~\ref{prop:hautus-cross-moment-high-prob}, we prove that
  \[
    |\sigma_{\min}(\widehat{\mat{P}}_\lambda(T)) - \sigma_{\min}(\mat{P}_\lambda)| = \mathcal{O}\left(\tfrac{1}{\sqrt{T}}\right),
  \]
  where $\mat{P}_\lambda:=[\mat{A}-\lambda \mat{I}, \, \mat{B}]$ and $\widehat{\mat{P}}_\lambda(T)$ is the data-driven estimate from the noisy model. In other words, the estimation error of the Hautus test decays at the rate $\mathcal{O}(T^{-1/2})$ as the observation horizon $T$ increases.
\end{itemize}
