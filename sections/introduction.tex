...
An equivalent characterization is given in terms of the Hautus (PBH) test. 
Recall that for the continuous-time LTI system~\eqref{eq:lti-intro}, the classical Hautus (Popov--Belevitch--Hautus, PBH) test \cite[Theorem~3.13]{trentelmanControlTheoryLinear2001} characterizes controllability and stabilizability as follows:
\begin{align*}
...
$$
Therefore, assuming access to $u(\cdot)$ and $x(\cdot)$ on a finite horizon $[0,T]$, one can estimate the rank of the Hautus matrix from data without explicitly identifying $(\mat{A},\mat{B})$. However, the quality of this estimate depends on the choice of input $u$. So, the main question we address in this paper is: \emph{how to design inputs $u$ that reliably reveal the controllability rank from data?}
\subsection{Main Contributions}
\begin{itemize}
  \item \textbf{A continuous-time data-driven Hautus margin.}
  Complementing terminal-state guarantees, we provide a continuous-time, data-driven analogue of the classical Hautus (PBH) test \cite{trentelmanControlTheoryLinear2001}.
  Defining the residual Gramian
  \[
    \mat G_\lambda(u):=\int_0^T(\dot x(t)-\lambda x(t))(\dot x(t)-\lambda x(t))^*\,\d t,
  \]
  we establish the equivalence
  \[
    (\mat{A},\mat{B}) \text{ is controllable}\quad\Leftrightarrow\quad
    \operatorname{rank}(\mat{G}_\lambda(u))=n\quad \forall\lambda\in\CC.
  \]
  We proof that we only have to check a finite number of $\lambda\in \Lambda$.We further develop a derivative-free formulation using Fourier transform.
  ...
  \]
  where $\mat{P}_\lambda:=[\mat{A}-\lambda \mat{I}, \, \mat{B}]$, $\hat{\mat{P}}_\lambda(T)$ is the data-driven estimate, $z(t):=[x(t);u(t)]\in\R^{n+m}$ is the stacked state-input vector and $\bar{\mat{S}}_Z(T):=\frac{1}{T}\int_0^T [x(t);u(t)][x(t)^\top,u(t)^\top]\,\d t$ is its normalized Gramian.
  Notably, this bound is uniform over all $\lambda\in\CC$ (Proposition~\ref{prop:hautus-cross-moment-high-prob}).
...
\begin{equation}\label{eq:lti}
  \dot x(t) = \mat{A} x(t) + \mat{B} u(t),\quad x(0) = x_0,
\end{equation}
...
Given an initial condition $x(0)\in\R^n$, the state trajectory admits the closed-form solution
...
More precisely, the reachable (controllable) subspace from the origin is the image of the Kalman controllability matrix
\begin{equation}\label{eq:controllability-matrix}
  \mat{C} := [\mat{B}, \; \mat{A}\mat{B}, \; \mat{A}^2\mat{B}, \; \cdots, \; \mat{A}^{n-1}\mat{B}]\in\R^{n\times nm}.
\end{equation}
Let $r:=\operatorname{rank}(\mat{C})$ denote the dimension of this subspace~\cite{kalmanMathematicalDescriptionLinear1963a,chenLinearSystemTheory1999}; the pair $(\mat{A},\mat{B})$ is said to be \emph{controllable} if and only if $r=n$.
Our objective is to design input signals $u(t)$ such that the observed trajectories reveal the controllability rank~$r$ from finitely many state snapshots.
When only output measurements $y(t)=\mat{C}_y x(t)$ are available (with a known output matrix $\mat{C}_y\in\R^{p\times n}$), the same rank analysis applies to the output-restricted controllability matrix $\mat{C}_y\mat{C}$.
