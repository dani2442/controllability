...
In particular, for linear systems, a fundamental prerequisite for many synthesis and certification tasks is that the data reveals the system's \emph{controllable subspace} and, ideally, certifies controllability \cite{kalmanMathematicalDescriptionLinear1963a,ControllabilityObservabilityStability}.
...
\begin{equation}\label{eq:hautus-classical}
  ...
  \end{aligned}
\end{equation}
The data-driven Hautus test estimates the rank of the matrix using the following identity:
$$
  [\mat{A}-\lambda \mat{I},\, \mat{B}]\begin{bmatrix}x(t)\\u(t)\end{bmatrix} = \dot{x}(t) - \lambda x(t)
$$
Therefore, assuming access to $u(\cdot)$ and $x(\cdot)$ on a finite horizon $[0,T]$, one can estimate the rank of the Hautus matrix from data without explicitly identifying $(\mat{A},\mat{B})$.
To recover the rank property \textit{from data}, one can form the cross-moment, defined by
\[
  \mat H_\lambda(T):=\int_0^T(\dot x(t)-\lambda x(t))z(t)^\top\,\d t
    = [\mat{A}-\lambda \mat{I},\, \mat{B}] \underbrace{\int_0^T z(t)z(t)^\top\,\d t}_{\mat{S}_Z(T)} \in \mathbb{C}^{n\times (n+m)},
\]
where $z(t):=[x(t);u(t)]\in\R^{n+m}$ is the stacked state-input vector.
The idea is that, if the Gramian $\mat{S}_Z(T)$ is invertible and well-conditioned, then the rank of $\mat H_\lambda(T)$ reveals the rank of the Hautus matrix.
However, the quality of this estimate depends strongly on the choice of input $u$.
...
\begin{itemize}
  \item \textbf{A data-driven controllability test.}
        We provide a \textit{continuous-time} and \textit{data-driven} analogue of the Hautus test:
        \begin{equation*}
          (\mat{A},\mat{B}) \text{ is controllable}\quad\Leftrightarrow\quad
          \operatorname{rank}(\mat{H}_\lambda(T))=n\quad \forall\lambda\in\CC,
        \end{equation*}
        assuming $\mat{S}_Z(T)$ is invertible.
        It suffices to check a finite set $\lambda \in \sigma(\mat{K})$, where
        \[
          \mat{K}:=\Big(\int_0^T x(t)x(t)^*\,\d t\Big)^{-1}\Big(\int_0^T x(t)\dot x(t)^*\,\d t\Big).
        \]
        We further develop a derivative-free formulation using Fourier methods.

  \item \textbf{Input design for conditioning the Hautus test.}
        We provide sharp, model-agnostic designs for conditioning the input, which improves the data-driven test.\\
        ...
        (2) Under an $H^1$ smoothness budget $\|u\|_{H^1}\le 1$, the optimal design is given by
        \[
          \begin{aligned}
            u(t) & = \sqrt{\alpha}\,Q
            \begin{bmatrix}
              \psi_0(t) \\ \vdots\\ \psi_{m-1}(t)
            \end{bmatrix},
            \qquad \text{where}\qquad
            \psi_k(t) =
            \begin{cases}
              \tfrac{1}{\sqrt{T}},                                  & k=0,    \\[6pt]
              \sqrt{\tfrac{2}{T}}\cos\!\big(\tfrac{k\pi t}{T}\big), & k\ge 1.
            \end{cases}
          \end{aligned}
        \]
        with $Q\in\R^{m\times m}$ an orthogonal matrix and $\alpha$ normalization term (Proposition~\ref{prop:hautus-isotropic-input-h1}).
        ...
        % \]
        % where $\mat{P}_\lambda:=[\mat{A}-\lambda \mat{I}, \, \mat{B}]$, $\widetilde{\mat{P}}_\lambda(T)$ is the data-driven estimate and $\bar{\mat{S}}_Z(T):=\frac{1}{T}\int_0^T z(t)z(t)^\top\,\d t$ is its normalized Gramian.

  \item \textbf{Concentration bounds under It\^o noise.} Consider the noisy LTI model given by
        \[
          \d x(t) = \big(\mat{A}x(t) + \mat{B}u(t)\big)\,\d t + \boldsymbol{\beta}\,\d W(t),
        \]
        where $W$ is a Brownian motion. In Proposition~\ref{prop:hautus-cross-moment-high-prob}, we prove
        \[
          |\sigma_{\min}(\widetilde{\mat{P}}_\lambda(T)) - \sigma_{\min}(\mat{P}_\lambda)| = \mathcal{O}\left(\tfrac{1}{\sqrt{T}}\right),
        \]
        where $\mat{P}_\lambda:=[\mat{A}-\lambda \mat{I}, \, \mat{B}]$ and $\widetilde{\mat{P}}_\lambda(T)$ is the data-driven estimate from the noisy model. The estimation error of the Hautus test decays as the observation horizon $T$ increases.
\end{itemize}
...
Section~\ref{sec:problem} introduces the deterministic residual cross-moment $\mat{H}_\lambda(T)$, proves the continuous-time data-driven Hautus test, and provides model-agnostic input-conditioning results; it also presents an operator formulation that reduces checking all $\lambda\in\CC$ to finitely many candidate values.
Section~\ref{sec:stochastic} develops the It\^o cross-moment estimator and concentration bounds, and discusses a derivative-free Fourier approximation.
Section~\ref{sec:experiments} reports numerical experiments.
Appendix~\ref{app:proofs} contains the proofs.
