...
\subsection{A Laplace-domain approximation without derivatives.}
The cross-moment in \eqref{eq:hautus-cross-moment-H} can also be approximated in the Laplace domain without forming $\dot x$.
For $s\in\CC$ with $\Re(s)>0$, define the (truncated) Laplace transform of the residual increment
\begin{equation*}
  \widehat m_\lambda(s)
  :=
  \int_0^T e^{-st}\,\d m_\lambda(t)
  =
  \int_0^T e^{-st}\,\d x(t)-\lambda\widehat x(s),
  \qquad
  \widehat x(s):=\int_0^T x(t)e^{-st}\,\d t.
\end{equation*}
Since $t\mapsto e^{-st}$ is of bounded variation, It\^o integration by parts yields
\[
  \int_0^T e^{-st}\,\d x(t)
  =
  x(T)e^{-sT}-x(0)+s\int_0^T x(t)e^{-st}\,\d t,
\]
hence
\begin{equation}\label{eq:hautus-laplace-y}
  \widehat m_\lambda(s)
  =
  x(T)e^{-sT}-x(0)+(s-\lambda)\widehat x(s).
\end{equation}
The advantage of the Laplace formulation is that the exponential decay $e^{-\Re(s)t}$ ensures integrability for $\Re(s)>0$, avoiding the windowing issues of the Fourier transform.
Define also $\widehat z(s):=\int_0^T z(t)e^{-st}\,\d t$.
For a discrete grid of Laplace frequencies $\{s_k\}_{k=1}^K$ with $\Re(s_k)>0$, the cross-moment and Gramian can be approximated by numerical quadrature:
\begin{equation}\label{eq:hautus-laplace-quadrature}
  \widetilde{\mat{H}}_\lambda^{\mathrm{Lap}}:=\sum_{k=1}^K w_k\,\widehat m_\lambda(s_k)\widehat z(s_k)^*,
  \qquad
  \widetilde{\mat{S}}_Z^{\mathrm{Lap}}:=\sum_{k=1}^K w_k\,\widehat z(s_k)\widehat z(s_k)^*,
\end{equation}
where $\{w_k\}$ are appropriate quadrature weights.
The Laplace transform $\widehat x(s)$ is computed by discretizing the integral $\int_0^T x(t)e^{-st}\,\d t\approx\sum_{j=0}^{N-1}x(t_j)e^{-st_j}\Delta t$.
If $x$ is absolutely continuous, Parseval's theorem on the imaginary axis gives
\[
  \mat{H}_\lambda(T)=\int_{\R}\widehat m_\lambda(\sigma+\ii\omega)\widehat z(\sigma+\ii\omega)^*\,\frac{\d\omega}{2\pi},
  \qquad
  \mat{S}_Z(T)=\int_{\R}\widehat z(\sigma+\ii\omega)\widehat z(\sigma+\ii\omega)^*\,\frac{\d\omega}{2\pi},
\]
for any fixed $\sigma>0$, relating the Laplace-domain computation to the time-domain cross-moments.
